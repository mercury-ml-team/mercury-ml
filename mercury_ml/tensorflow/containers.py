...
class ModelSavers:
    from mercury_ml.tensorflow.providers import model_saving
...
class ModelLoaders:
    from mercury_ml.tensorflow.providers import model_loading
    load_hdf5 = model_loading.load_hdf5_model

class LossFunctionFetchers:
    from mercury_ml.tensorflow.providers import loss_function_fetching
...
class OptimizerFetchers:
    from mercury_ml.tensorflow.providers import optimizer_fetching
    get_keras_optimizer = optimizer_fetching.get_keras_optimizer

class ModelCompilers:
    from mercury_ml.tensorflow.providers import model_compilation
    compile_model = model_compilation.compile_model


class ModelFitters:
    from mercury_ml.tensorflow.providers import model_fitting
    fit = model_fitting.fit
    fit_generator = model_fitting.fit_generator

class ModelDefinitions:
    from mercury_ml.tensorflow.providers.model_definition import conv_simple, mlp_simple
...
class GeneratorPreprocessingFunctionGetters:
    from mercury_ml.tensorflow.providers.generator_preprocessors import get_random_eraser
    get_random_eraser = get_random_eraser


class CallBacks:
    from mercury_ml.tensorflow.providers.model_callbacks import  TensorBoardProvider, \
...
class ModelEvaluators:
    from mercury_ml.tensorflow.providers import model_evaluation
    evaluate = model_evaluation.evaluate
    evaluate_generator = model_evaluation.evaluate_generator

class PredictionFunctions:
    from mercury_ml.tensorflow.providers import prediction
